...
// tony/serverHelper.js
var import_url = require("url");
function request(req) {
  const parsed = (0, import_url.parse)(req.url);
  console.log(parsed);
  req.path = parsed.pathname;
  req.query = parsed.query;
}
function response(res) {
  res.sendFile = function(path2) {
    const absPath = path2.resolve(filePath);
    fs.createReadStream(absPath).on("error", () => {
      res.writeHead(404);
...
function serveStatic(root) {
  return async function serveStaticMiddleware(req, res) {
    ...
    const filePath2 = import_path.default.join(process.cwd(), root, urlPath);
    try {
      const stat = await import_fs.default.promises.stat(filePath2);
      if (stat.isDirectory()) return true;
      const range = req.headers.range;
      const contentType = src_default.getType(filePath2) || "application/octet-stream";
      ...
      if (range) {
        ...
        import_fs.default.createReadStream(filePath2, { start, end }).pipe(res);
        return false;
      }
      res.writeHead(200, {
      ...
      if (req.method === "HEAD") {
        ...
        import_fs.default.createReadStream(filePath2).pipe(res);
      }
      return false;
    } catch (err) {
      return true;
...
import_http.default.createServer(async (req, res) => {
  ...
  if (!await cookies(req, res)) return false;
